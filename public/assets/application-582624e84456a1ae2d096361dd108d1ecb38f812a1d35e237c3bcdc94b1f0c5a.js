...
function mapCenterZoom(geoJSONData) {
  ...
  let boundN = (maxLats.length > 0) ? Math.max.apply(Math, maxLats) : defaultValues['bounds']['n'];
  let boundS = (minLats.length > 0) ? Math.min.apply(Math, minLats) : defaultValues['bounds']['s'];
  ...
  if (lonStartValues.length > 0) {
    ...
    console.log('W-E', boundW, boundE);
    console.log('N-S', boundN, boundS);
  }
