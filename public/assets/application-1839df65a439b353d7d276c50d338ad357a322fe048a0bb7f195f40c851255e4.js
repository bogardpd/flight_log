...
function mapCenterZoom(geoJSONData) {
  ...
  let minRouteCount = Math.min(...lonRouteCount.values())
  lonStartValues = lonStartValues.filter(l => l[2] == minRouteCount)
  ...
  // Determine bounds.
  if (minRouteCount == 0) {
    singleBound = lonStartValues[0][0] + (lonStartValues[0][1] / 2);
    while (singleBound >= 180) {
      singleBound = singleBound - 360;
    }
    boundW = singleBound;
    boundE = singleBound + 360;
  } else {
    boundW = lonStartValues[0][0];
    boundE = (lonStartValues[0][1] / 2);
  }
