module FlightsHelper
  ...
  def display_airline_by_code(iata_code)
    ...
    if airline.length > 0
      html = %Q(#{code_mono(iata_code)}<div class="supplemental_info">#{airline.first.airline_name}#{image_tag(airline_icon_path(iata_code), alt: iata_code, title: airline.first.airline_name, class: 'airline_icon icon_right')}</div>) 
    else
